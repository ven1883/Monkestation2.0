...
/datum/action/cooldown/spell/sacrament/cast(atom/cast_on)
	...
	sound_to_playing_players('monkestation/sound/ambience/antag/darkspawn/magic/sacrament_begin.ogg', 50, FALSE, pressure_affected = FALSE)
	...
	notify_ghosts("Darkspawn [user.real_name] has begun the Sacrament at [get_area(user)]! ", source = user, ghost_sound = 'monkestation/sound/ambience/antag/darkspawn/magic/devour_will_victim.ogg', alert_overlay = alert_overlay, action = NOTIFY_ORBIT)
	...
	for(var/stage in 1 to 2)
		...
		switch(stage)
			if(1)
				...
				sound_to_playing_players('monkestation/sound/ambience/antag/darkspawn/magic/sacrament_01.ogg', 20, FALSE, pressure_affected = FALSE)
			if(2)
				...
				sound_to_playing_players('monkestation/sound/ambience/antag/darkspawn/magic/sacrament_02.ogg', 20, FALSE, pressure_affected = FALSE)
	...
	for(var/turf/T in range(10, owner))
		if(prob(35))
			addtimer(CALLBACK(src, PROC_REF(unleashed_psi), \), rand(1, 40))
...
/datum/action/cooldown/spell/sacrament/proc/finalize(mob/living/carbon/human/user)
	...
	sound_to_playing_players('monkestation/sound/ambience/antag/darkspawn/magic/sacrament_ending.ogg', 75, FALSE, pressure_affected = FALSE)
	QDEL_IN(soundloop, 1 SECONDS)
	addtimer(CALLBACK(darkspawn, TYPE_PROC_REF(/datum/antagonist/darkspawn, sacrament)), 1 SECONDS)

/datum/action/cooldown/spell/sacrament/proc/unleashed_psi(turf/current_position)
	if(!in_use)
		return
	playsound(current_position, 'monkestation/sound/ambience/antag/darkspawn/magic/divulge_end.ogg', 25, FALSE)
	new/obj/effect/temp_visual/revenant/cracks/glow(current_position)
