...
/datum/action/cooldown/spell/shapeshift/crawling_shadows
	...
	sound = '/magic/devour_will_end.ogg'
...
/datum/action/cooldown/spell/simulacrum/cast(atom/cast_on)
	...
	if(!isliving(owner))
		return
	var/mob/living/host = owner
	host.balloon_alert(host, "Zkxa'ya")
	host.visible_message(span_warning("[host] breaks away from [host]'s shadow!"), span_velvet("You create an illusion of yourself."))
	playsound(host, '/magic/devour_will_form.ogg', 50, 1)
...
/datum/action/cooldown/spell/fray_self/proc/fray()
	...
	var/list/consenting_candidates = SSpolling.poll_ghost_candidates(
		"Would you like to play as piece of [caster]'s psyche?",
		check_jobban = "Darkspawn",
		role = ROLE_DARKSPAWN,
		poll_time = 10 SECONDS,
		alert_pic = /datum/antagonist/traitor/contractor,
		role_name_text = "darkspawn psyche"
	)
	...
	playsound(caster, '/magic/devour_will_form.ogg', 50, 1)
