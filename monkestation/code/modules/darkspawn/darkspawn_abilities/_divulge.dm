...
/datum/action/cooldown/spell/divulge/cast(atom/cast_on)
	...
	notify_ghosts("Darkspawn [user.real_name] has begun divulging at [get_area(user)]! ", source = user, ghost_sound = 'monkestation/sound/ambience/antag/darkspawn/magic/devour_will_victim.ogg', alert_overlay = alert_overlay, action = NOTIFY_ORBIT)
	...
	for(var/stage in 1 to 3)
		...
		switch(stage)
			if(1)
				...
				playsound(user, 'monkestation/sound/ambience/antag/darkspawn/magic/divulge_01.ogg', 30, 0)
			if(2)
				...
				playsound(user, 'monkestation/sound/ambience/antag/darkspawn/magic/divulge_02.ogg', 40, 0)
			if(3)
				...
				playsound(user, 'monkestation/sound/ambience/antag/darkspawn/magic/divulge_03.ogg', 50, 0)
	...
	if(isethereal(user))//keep the light disabled
		user.emp_act(EMP_LIGHT)
	playsound(user, 'monkestation/sound/ambience/antag/darkspawn/magic/divulge_ending.ogg', 50, 0)
	...
	playsound(user, 'monkestation/sound/ambience/antag/darkspawn/magic/divulge_end.ogg', 50, 0)
	...
	for(var/mob/living/target in view(7, user))
		if(is_team_darkspawn(target) || target == user) //probably won't have thralls yet, but might as well check just in case
			continue
